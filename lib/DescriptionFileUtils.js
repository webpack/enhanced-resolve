...
const INHERIT_PROPERTIES = ["version", "name", "sideEffects"];

/**
 * @param {Object} child child description file
 * @param {Object} parent parent description file
 * @param {string} directory parent description file directory
 */
function inheritParentProperties(child, parent, directory) {
	let updated = false;

	for (const prop of INHERIT_PROPERTIES)
		if (!(prop in child) && prop in parent) {
			child[prop] = parent[prop];
			updated = true;
		}

	if (updated) child._parentDescriptionFileRoot = directory;
}
