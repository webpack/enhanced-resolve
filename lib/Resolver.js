...
class Resolver {
	 ...
	 */
	isDirectory(path) {
		// Called for every parse() step; a single char-code comparison is
		// cheaper than `String#endsWith` (which goes through
		// `%StringSlice`/`%StringEquals` internally even for a single-char
		// suffix) and matters under CodSpeed instrumentation.
		const len = path.length;
		return len > 0 && path.charCodeAt(len - 1) === 47; /* "/" */
	}
