...
class Resolver {
	...
	resolve(context, path, request, resolveContext, callback) {
		...
		if (resolveContext.log) {
			...
			/** @type {string[]} */
			const log = [];
			this.doResolve(this.hooks.resolve, obj, message, {
				log: (msg) => {
					parentLog(msg);
					log.push(msg);
				},
	 ...
	 * @param {((err?: null | Error, result?: ResolveRequest) => void)=} callback optional legacy callback; when provided it is invoked with the resolved value or error
	 * @returns {Promise<ResolveRequest | null | undefined> | void} a promise that resolves to the request or undefined; returns void when `callback` is provided
	 */
	doResolve(hook, request, message, resolveContext, callback) {
		const promise = this._doResolvePromise(
			hook,
			request,
			message,
			resolveContext,
	 ...
	 * @param {null | string} message string
	 * @param {ResolveContext} resolveContext resolver context
	 * @returns {Promise<ResolveRequest | null | undefined>} resolved request or undefined when the hook chain should continue
	 */
	async _doResolvePromise(hook, request, message, resolveContext) {
		...
		if (hook.isUsed()) {
			...
			const result = await hook.promise(request, innerContext);
