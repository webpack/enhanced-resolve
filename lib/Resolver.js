...
class Resolver {
	...
	constructor(fileSystem, options) {
		...
		const maxCacheSize =
			options &&
			options.unsafeCache &&
			options.unsafeCache instanceof ResolveCache
				? options.unsafeCache.maxSize
				: 0;
		this.join = createCachedJoin(maxCacheSize);
		this.dirname = createCachedDirname(maxCacheSize);
		/** @type {KnownHooks} */
		this.hooks = {
