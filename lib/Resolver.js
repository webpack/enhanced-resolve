...
Resolver.prototype.join = function join(path, request) {
	if(absoluteRegExp.test(request)) return request;
	if(path == "/") return path + request;
...
var parentDirectoryWinEndRegExp = /\\[^\\]+\\\.\.$/g;
var parentDirectoryNixMiddleRegExp = /\/[^\/]+\/\.\.\//;
var parentDirectoryNixEndRegExp = /\/[^\/]+\/\.\.$/g;
Resolver.prototype.normalize = function normalize(path) {
	...
	while(parentDirectoryWinEndRegExp.test(path)) {
	...
	while(parentDirectoryNixEndRegExp.test(path)) {
