 ...
 * @property {(rootPath: string, request: string) => string} fn cached join function
 * @property {Map<string, Map<string, string | undefined>>} cache the underlying cache map
 */

/**
 * @typedef {object} DirnameCacheEntry
 * @property {(maybePath: string) => string} fn cached dirname function
 * @property {Map<string, string>} cache the underlying cache map
 */

/**
 * @typedef {object} PathCacheFunctions
 * @property {PathCacheEntry} join cached join
 * @property {DirnameCacheEntry} dirname cached dirname
 * @property {(type?: "join" | "dirname") => void} clear clear caches
 */
...
class Resolver {
	...
	constructor(fileSystem, options) {
		...
		if (!pathCache) {
			const joinEntry = createCachedJoin();
			const dirnameEntry = createCachedDirname();
			pathCache = {
				join: joinEntry,
				dirname: dirnameEntry,
				/**
				 * @param {"join" | "dirname"=} type specific cache to clear, or all if omitted
				 */
				clear(type) {
					if (!type || type === "join") joinEntry.cache.clear();
					if (!type || type === "dirname") dirnameEntry.cache.clear();
				},
			};
