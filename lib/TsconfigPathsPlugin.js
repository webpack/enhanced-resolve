...
function tsconfigPathsToResolveOptions(paths, baseUrl) {
	...
	for (const pattern of sortedKeys) {
		...
		const absolutePaths = mappings.map((mapping) => join(baseUrl, mapping));
	...
	if (baseUrl && !modules.includes(baseUrl)) {
		modules.push(baseUrl);
	}
...
module.exports = class TsconfigPathsPlugin {
	...
	async _loadTsconfigPathsMap(fileSystem, absTsconfigPath) {
		...
		const main = tsconfigPathsToResolveOptions(paths, absoluteBaseUrl);
		...
		for (const ref of references) {
			...
			try {
				...
				if (refConfig.compilerOptions && refConfig.compilerOptions.paths) {
					const refBaseUrl = refConfig.compilerOptions.baseUrl || ".";
					const refContext = dirname(refConfigPath);
					const absoluteBaseUrl = getAbsoluteBaseUrl(refContext, refBaseUrl);
