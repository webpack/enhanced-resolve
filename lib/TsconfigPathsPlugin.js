...
module.exports = class TsconfigPathsPlugin {
	...
	_selectPathsDataForContext(requestPath, tsconfigPathsMap) {
		...
		// Per-map memoization: a real build resolves many imports from
		// the same source file (so the same `requestPath` arrives here
		// repeatedly), and the answer depends only on `(map, requestPath)`.
		// Skip the full `contextList` scan on the hot path by caching the
		// previously-computed selection.
		let perMap = _contextSelectionCache.get(tsconfigPathsMap);
		if (perMap !== undefined) {
