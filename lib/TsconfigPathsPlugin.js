...
module.exports = class TsconfigPathsPlugin {
	...
	async _loadTsconfigPathsMap(resolver, absTsconfigPath) {
		...
		const mainOwn = tsconfigPathsToResolveOptions(
			mainContext,
		...
		// Give references priority over the main tsconfig's `paths`: when a
		// request resolves against the main context, try every reference
		// alias first, then fall back to main's own paths. Modules (baseUrl
		// candidates) stay scoped to main — those are context-sensitive and
		// each reference keeps its own baseUrl in its own data.
		const refContexts = Object.keys(refs);
		let main = mainOwn;
		if (refContexts.length > 0) {
			/** @type {import("./AliasUtils").CompiledAliasOption[]} */
			const combinedAlias = [];
			for (let i = 0; i < refContexts.length; i++) {
				const refAlias = refs[refContexts[i]].alias;
				for (let j = 0; j < refAlias.length; j++) {
					combinedAlias.push(refAlias[j]);
				}
			}
			for (let i = 0; i < mainOwn.alias.length; i++) {
				combinedAlias.push(mainOwn.alias[i]);
			}
			main = { alias: combinedAlias, modules: mainOwn.modules };
		}
