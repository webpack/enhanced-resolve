 ...
 *
 * Only the top-level values are descended into, never the field's own keys:
 * in both forms of `exports` and in `imports` those values are targets, while
 * the keys are either subpaths or - for the `exports` shorthand, where the
 * whole field is one conditional mapping - condition names that
 * `buildExportsField` has already checked.
...
module.exports.explainSubpathsInConditions =
	function explainSubpathsInConditions(field, isExports) {
		...
		return `the value at ${found.trail
			.map(quote)
			.join(" -> ")} is an object with subpath keys (${subpathKeys
			.map(quote)
			.join(
				", ",
