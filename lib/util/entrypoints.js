...
/**
 * Subpath keys nested inside a conditional export can never match, so fail
 * fast with an explanation instead of silently resolving to a wrong target
 * @param {MappingValue} value conditional export value
 * @param {string} condition condition name the value belongs to
 */
function assertConditionalExportKeys(value, condition) {
	if (value === null || typeof value === "string") return;

	if (Array.isArray(value)) {
		for (let i = 0; i < value.length; i++) {
			assertConditionalExportKeys(value[i], condition);
		}
		return;
	}
	...
	for (let i = 0; i < keys.length; i++) {
		...
		if (key.charCodeAt(0) === dotCode) {
			throw new Error(
				`Conditional exports can't contain subpath keys (found ${JSON.stringify(
					key,
				)} in the conditional export ${JSON.stringify(
					condition,
				)}). Subpaths must be top-level keys of the exports field, with conditions nested inside them`,
			);
		}

		assertConditionalExportKeys(value[key], key);
	}
}
		...
		if (key.charCodeAt(0) !== dotCode) {
			...
			if (i === 0) {
				...
				for (let j = 0; j < keys.length; j++) {
					assertConditionalExportKeys(
						/** @type {ConditionalMapping} */ (field)[keys[j]],
						keys[j],
					);
				}
