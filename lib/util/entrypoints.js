...
/**
 * Compare two pattern keys by specificity. Pattern indices can be passed
 * in when the caller already computed them (see `findMatch`) — that
 * short-circuits the two `indexOf("*")` calls the old signature used,
 * which ran redundantly for every key iteration in the match loop.
 * @param {string} a first string
 * @param {number} aPatternIndex precomputed `a.indexOf("*")` or -1
 * @param {string} b second string
 * @param {number} bPatternIndex precomputed `b.indexOf("*")` or -1
 * @returns {number} compare result
 */
function patternKeyCompare(a, aPatternIndex, b, bPatternIndex) {
...
function findMatch(request, field) {
	...
	// Track the match branch during the scan so we can skip the two extra
	// `bestMatch.endsWith("/")` / `bestMatch.includes("*")` scans after the
	// loop — they were redundant with the branch we already took here.
	let bestMatchIsPattern = false;
	let bestMatchIsSubpath = false;
	// Track bestMatch's pattern index so `patternKeyCompare` doesn't have to
	// scan the same string again every iteration. -1 means "no pattern yet".
	let bestMatchPatternIndex = -1;
	...
	for (let i = 0; i < keys.length; i++) {
		...
		if (patternIndex !== -1 && request.startsWith(key.slice(0, patternIndex))) {
			...
			if (
				...
				patternKeyCompare(
					bestMatch,
					bestMatchPatternIndex,
					key,
					patternIndex,
				) === 1 &&
				// Confirm `key` has a single "*" — scan forward from the one
				// we already found rather than doing a full `lastIndexOf`.
				!key.includes("*", patternIndex + 1)
			) {
				bestMatch = key;
				bestMatchPatternIndex = patternIndex;
				bestMatchSubpath = request.slice(
					patternIndex,
					request.length - patternTrailer.length,
				);
				bestMatchIsPattern = true;
				bestMatchIsSubpath = false;
			}
		}
		// For legacy `./foo/`
		else if (
			key.charCodeAt(key.length - 1) === slashCode &&
			request.startsWith(key) &&
			patternKeyCompare(bestMatch, bestMatchPatternIndex, key, patternIndex) ===
				1
		) {
			bestMatch = key;
			bestMatchPatternIndex = patternIndex;
			bestMatchSubpath = request.slice(key.length);
			bestMatchIsPattern = false;
			bestMatchIsSubpath = true;
		}
	}
...
function conditionalMapping(conditionalMapping_, conditionNames) {
	...
	loop: while (lookup.length > 0) {
		// Keep a local reference to the stack top so the two `top[2] = i+1`
		// writes below don't have to re-walk `lookup.length - 1` each time.
