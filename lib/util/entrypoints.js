...
/**
 * Looks for a conditional mapping whose keys are subpaths - the "conditions
 * wrapping subpaths" shape from
 * https://github.com/webpack/enhanced-resolve/issues/325, which Node.js does
 * not support: the subpaths are read as condition names and match nothing.
 *
 * Diagnostic only, and deliberately not part of building the field. Walking
 * every target of every package.json would cost a full tree walk per package
 * to catch a mistake almost none of them make, and rejecting the field
 * outright would break packages that resolve today under some conditions.
 * Callers run this once a request has already failed, where the walk is free
 * relative to the error being raised anyway.
 * @param {MappingValue} mapping mapping in target position
 * @param {number} subpathCode leading char code that marks a subpath key
 * @param {string[]} trail keys walked from the field root down to `mapping`
 * @returns {{ trail: string[], keys: string[] } | null} offending mapping
 */
function findSubpathsInConditions(mapping, subpathCode, trail) {
	...
	if (Array.isArray(mapping)) {
		for (let i = 0; i < mapping.length; i++) {
			const found = findSubpathsInConditions(mapping[i], subpathCode, trail);
	...
	for (let i = 0; i < keys.length; i++) {
		if (keys[i].charCodeAt(0) === subpathCode) {
...
/**
 * Explains why nothing matched, when the reason is a conditional mapping that
 * wraps subpaths. Returns `null` when the field does not have that shape, so
 * the caller keeps its ordinary "not exported" wording.
 * @param {ExportsField | ImportsField | null | undefined} field the field
 * @param {boolean} isExports true for `exports`, false for `imports`
 * @returns {string | null} explanation to append to the error, if any
 */
module.exports.explainSubpathsInConditions =
	function explainSubpathsInConditions(field, isExports) {
		...
		/** @type {string[]} */
		const trail = [];
		/** @type {{ trail: string[], keys: string[] } | null} */
		let found = null;

		// `exports` may be the shorthand where the whole field is one
		// conditional mapping for "."; then the field itself is a target.
		if (isExports && keys.length > 0 && keys[0].charCodeAt(0) !== dotCode) {
			found = findSubpathsInConditions(field, subpathCode, trail);
		} else {
			for (let i = 0; i < keys.length && !found; i++) {
				trail.push(keys[i]);
				found = findSubpathsInConditions(
					/** @type {Record<string, MappingValue>} */ (field)[keys[i]],
					subpathCode,
					trail,
				);
				trail.pop();
		...
		const quote = (/** @type {string} */ key) => JSON.stringify(key);
		const subpathKeys = found.keys.filter(
			(key) => key.charCodeAt(0) === subpathCode,
		);
		const where =
			found.trail.length > 0
				? `the value at ${found.trail.map(quote).join(" -> ")}`
				: `the ${isExports ? "exports" : "imports"} field`;
		const condition =
			found.trail.length > 0
				? found.trail[found.trail.length - 1]
				: isExports
					? "import"
					: "node";
		const example = subpathKeys
			.map((key) => `${quote(key)}: { ${quote(condition)}: ... }`)
			.join(", ");

		return `${where} is an object with subpath keys (${subpathKeys
			.map(quote)
			.join(
				", ",
			)}), which is not supported - a condition can only map to a target or to further conditions, so those subpaths never match. Put the subpaths at the top level and nest the conditions inside them instead, e.g. { ${example} }.`;
	};
