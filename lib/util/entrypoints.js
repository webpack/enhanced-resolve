 ...
 * @returns {FieldKeyInfo[]} precomputed per-key info sorted by `patternKeyCompare` descending (best-match-first; see `computeFindMatch`)
 */
function getFieldKeyInfos(field) {
	...
	// equivalent priority.
	infos.sort((a, b) => patternKeyCompare(a.key, b.key));
...
function computeFindMatch(request, field) {
	...
	// `infos` is sorted by `patternKeyCompare` descending (highest priority
	// first), so the first key that satisfies the prefix/suffix/length
	// checks is guaranteed to be the best match — we can break immediately
	// instead of evaluating `patternKeyCompare` against every remaining
	// candidate. For fields with many `./prefix*` style entries this turns
	// the inner scan from O(N) into O(position-of-best-match).
