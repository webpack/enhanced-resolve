...
/**
 * Bundle returned by `getFieldKeyInfos`. `sortedInfos` is the
 * priority-ordered key list used by the existing loop. `byCharAtN` /
 * `anyBucket` / `bucketCharPos` partition that list by the char at
 * the first position where exports / imports keys actually carry
 * information, so the no-match cold path can fast-reject a request
 * whose char at that position lands in no bucket — see the comment
 * in `computeFindMatch`.
 * @typedef {object} FieldKeyData
 * @property {FieldKeyInfo[]} sortedInfos priority-sorted info list (best-match-first)
 * @property {Map<number, FieldKeyInfo[]>} byCharAtN bucket of infos whose required-prefix char at `bucketCharPos` is the key
 * @property {FieldKeyInfo[]} anyBucket infos whose required prefix is too short to be bucketed at `bucketCharPos`
 * @property {number} bucketCharPos position to read from the request for bucket dispatch; `0` disables bucketing
 */
 ...
 * @type {WeakMap<RecordMapping, FieldKeyData>}
 */
 ...
 * @returns {FieldKeyData} precomputed per-key info plus a bucket map keyed by the first informative char (see `FieldKeyData`)
 */
function getFieldKeyInfos(field) {
	const fieldKey = /** @type {RecordMapping} */ (field);
	let data = _fieldKeyInfoCache.get(fieldKey);
	if (data !== undefined) return data;
	const keys = Object.getOwnPropertyNames(field);
	/** @type {FieldKeyInfo[]} */
	const sortedInfos = Array.from({ length: keys.length });
	...
	// Sort highest-priority-first so `computeFindMatch` can break on the
	// first satisfying candidate. The comparator is the same
	// `patternKeyCompare` previously evaluated inline per candidate (so it
	// returns -1 when `a` outranks `b`). Array.sort is stable in modern V8,
	// matching the original "first-seen wins" tiebreaker for keys with
	// equivalent priority.
	sortedInfos.sort((a, b) => patternKeyCompare(a.key, b.key));

	// Bucket by the char at the first position where exports / imports
	// keys actually carry information. Exports keys always start with
	// `./` so position 2 is the first such char (`u` for `./ui/*`, `d`
	// for `./data/*`, ...); imports keys always start with `#` so
	// position 1 is informative. Picking this from the first key's
	// first char skips re-discovering it per resolve and falls back to
	// a no-op (`bucketCharPos === 0`) for non-standard shapes.
	//
	// Keys whose required prefix is too short to read at
	// `bucketCharPos` (e.g. legacy `./` or wildcard `./*`) go into
	// `anyBucket` and must be considered for every request — when that
	// bucket is non-empty the fast-path is bypassed (rare in practice).
	let bucketCharPos = 0;
	if (sortedInfos.length > 0) {
		const firstChar = sortedInfos[0].key.charCodeAt(0);
		if (firstChar === dotCode) {
			bucketCharPos = 2;
		} else if (firstChar === hashCode) {
			bucketCharPos = 1;
		}
	}
	/** @type {Map<number, FieldKeyInfo[]>} */
	const byCharAtN = new Map();
	/** @type {FieldKeyInfo[]} */
	const anyBucket = [];
	if (bucketCharPos > 0) {
		for (let i = 0; i < sortedInfos.length; i++) {
			const info = sortedInfos[i];
			// The "required prefix" — the longest substring that must
			// appear at the start of any matching request — is the
			// wildcardPrefix for pattern keys and the key itself for
			// legacy subpath keys. Both have to be at least
			// `bucketCharPos + 1` chars long for the bucket char to
			// exist.
			const required = info.isPattern ? info.wildcardPrefix : info.key;
			if (required.length > bucketCharPos) {
				const code = required.charCodeAt(bucketCharPos);
				let bucket = byCharAtN.get(code);
				if (bucket === undefined) {
					bucket = [];
					byCharAtN.set(code, bucket);
				}
				bucket.push(info);
			} else {
				anyBucket.push(info);
	...
	data = { sortedInfos, byCharAtN, anyBucket, bucketCharPos };
	_fieldKeyInfoCache.set(fieldKey, data);
	return data;
}
...
function computeFindMatch(request, field) {
	...
	// `sortedInfos` is sorted by `patternKeyCompare` descending (highest
	// priority first), so the first key that satisfies the
	// prefix/suffix/length checks is guaranteed to be the best match —
	// we can break immediately instead of evaluating `patternKeyCompare`
	// against every remaining candidate. For fields with many
	// `./prefix*` style entries this turns the inner scan from O(N)
	// into O(position-of-best-match).
	//
	// Dispatch through the per-position bucket when the field is in a
	// regular shape (every required prefix is long enough to read at
	// `bucketCharPos`). When a key's required prefix is too short
	// (e.g. `./` or `./*` in an exports field, `#` in an imports field)
	// `anyBucket` is non-empty and we fall back to the full sorted list
	// to preserve priority ordering across keys that span both
	// partitions.
	const { sortedInfos, byCharAtN, anyBucket, bucketCharPos } =
		getFieldKeyInfos(field);
	/** @type {FieldKeyInfo[]} */
	let scan;
	if (
		bucketCharPos > 0 &&
		anyBucket.length === 0 &&
		requestLen > bucketCharPos
	) {
		const bucket = byCharAtN.get(request.charCodeAt(bucketCharPos));
	...
	for (let i = 0; i < scan.length; i++) {
		...
		if (patternIndex !== -1) {
			...
			break;
		} else if (info.isLegacySubpath && request.startsWith(key)) {
			bestMatch = key;
			bestMatchInfo = info;
...
/** @typedef {[string[], string | null]} ProcessorResult */

/**
 * Per-`(field, conditionNames, request)` memoization of the final
 * `fieldProcessor` output (`[paths, usedField]`). The lower-level caches
 * (`_findMatchCache`, `_conditionalMappingCache`) already memoize the
 * intermediate match and condition walk; this one captures the FULL
 * processor result so warm hits skip `directMapping` / `targetMapping`
 * entirely — those rebuild the output `string[]` (and substitute
 * wildcard remainders) on every call when not cached.
 *
 * Outer key is the field object (rebuilt with the description file, so
 * a `WeakMap` lets the cache fall away naturally). Middle key is the
 * `conditionNames` Set — stable for the resolver's lifetime, so the
 * second `WeakMap` makes the cache vanish when the resolver does.
 * Inner key is the raw `request` string; the inner `Map` grows with
 * unique specifiers but mirrors the long-lived shape of
 * `_findMatchCache`.
 * @type {WeakMap<RecordMapping, WeakMap<Set<string>, Map<string, ProcessorResult>>>}
 */
const _processResultCache = new WeakMap();
...
function createFieldProcessor(
	...
	const cacheKey = /** @type {RecordMapping} */ (field);
	return function fieldProcessor(request, conditionNames) {
		// Result cache: keyed by (field, conditionNames, request). On a
		// hit we return the exact tuple computed last time and skip the
		// `findMatch` / `directMapping` walk below. The lookup is two
		// WeakMap.get + one Map.get on the cache-hit fast path.
		let perCond = _processResultCache.get(cacheKey);
		/** @type {Map<string, ProcessorResult> | undefined} */
		let perReq;
		if (perCond !== undefined) {
			perReq = perCond.get(conditionNames);
			if (perReq !== undefined) {
				const cached = perReq.get(request);
				if (cached !== undefined) return cached;
			}
		}
		...
		// `assertRequest` (and `directMapping` via `assertTarget`) can
		// throw on malformed input; we only reach this point on success,
		// so caching the result here keeps thrown errors from being
		// memoized.
		if (perCond === undefined) {
			perCond = new WeakMap();
			_processResultCache.set(cacheKey, perCond);
		}
		if (perReq === undefined) {
			perReq = new Map();
			perCond.set(conditionNames, perReq);
		}
		perReq.set(request, result);
		return result;
	};
}
