...
function getFieldKeyInfos(field) {
	...
	// first char skips re-discovering it per resolve.
	//
	// Skip building the bucket when the field has only a handful of
	// keys — the upfront `new Map` + per-key array allocations cost
	// more than the per-resolve `startsWith` checks they save. Most
	// real-world `exports` fields are tiny (typical: `.`, a couple of
	// subpaths, `package.json`); the bucket-dispatch wins are
	// concentrated on the >10-key shapes seen in monorepo-style
	// libraries and the synthetic `exports-patterns-many` benchmark.
	// `anyBucket` is non-empty only when a key's required prefix is
	// too short to read at `bucketCharPos` (e.g. legacy `./`); in that
	// case the resolve hot path falls back to the sorted full list.
	/** @type {Map<number, FieldKeyInfo[]>} */
	const byCharAtN = new Map();
	/** @type {FieldKeyInfo[]} */
	const anyBucket = [];
	let bucketCharPos = 0;
	if (sortedInfos.length > 8) {
		...
		if (bucketCharPos > 0) {
			for (let i = 0; i < sortedInfos.length; i++) {
				...
				// The "required prefix" — the longest substring that
				// must appear at the start of any matching request — is
				// the wildcardPrefix for pattern keys and the key
				// itself for legacy subpath keys. Both have to be at
				// least `bucketCharPos + 1` chars long for the bucket
				// char to exist.
