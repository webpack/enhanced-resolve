...
/**
 * @param {PathType} pathType pathType
 * @returns {boolean} true if path is valid absolute or relative path
 */
function isValidAbsoluteOrRelativePath(pathType) {
	switch (pathType) {
		case PathType.AbsoluteWin:
		case PathType.AbsolutePosix:
		case PathType.Relative:
...
/**
 * enhanced-resolve should resolve paths
 * until it reach valid absolute or relative path
 * @param {string[]} paths paths
 * @returns {string[]} filters paths
 */
function filterImportsExportsFieldTargets(paths) {
	let k = paths.length;
	for (let i = 0; i < paths.length; i++) {
		const type = getType(paths[i]);
		if (isValidAbsoluteOrRelativePath(type)) {
			k = i + 1;
			break;
		}
	}
...
exports.filterImportsExportsFieldTargets = filterImportsExportsFieldTargets;
