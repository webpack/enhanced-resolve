...
/** @typedef {Record<string, MappingValue>|ConditionalMapping|DirectMapping} ExportField */
...
module.exports = function processExportsField(
	...
	const isArray = Array.isArray(exportsFieldDefinition);

	// handle syntax sugar, if export field is direct mapping for "."
	if (isArray || typeof exportsFieldDefinition === "string") {
		if (relativeToPackagePathRequest.length > 1) return [];

		return isArray
			? /** @type {string[]} */ (exportsFieldDefinition).slice()
			: [/** @type {string} */ (exportsFieldDefinition)];
	}
