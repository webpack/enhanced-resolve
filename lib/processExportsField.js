 ...
 * @callback ProcessExportsField
 * @param {ExportsField} exportsField exports field
...
/**
 * @returns {ProcessExportsField} process callback
 */
module.exports = function createProcessor() {
	/** @type {Map<ExportsField, PathTreeNode>} */
	const cache = new Map();
	return function processExportsField(
		exportsField,
		...
		let treeRoot = cache.get(exportsField);

		if (!treeRoot) {
			treeRoot = buildPathTree(exportsField);
			cache.set(exportsField, treeRoot);
		}
