...
Enhanced-resolve name keys ("." and "./feature") as exports.
...
Key in direct and conditional mapping is called condition name.
...
/**
 * Map request to strings divided by slash,
 * last element is a requested file
...
function tryToMatch(request, exportFieldKey) {
	...
	if (exportFieldKey.charCodeAt(1) !== slashCode) {
		throw new Error(
			exportFieldKey.charCodeAt(1) === dotCode
				? "Export field key reference out of package context"
				: 'Export field key should be relative path and start with "."'
		);
	}
