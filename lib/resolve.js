		...
		(sync?readdirSync:readdirAsync)(dirname, function(err, files) {
			...
			seqments.forEach(function(file) {
				...
				(sync?statSync:statAsync)(path.join(dirname, fullFilename), function(err, stat) {
					...
					// There is a special case if we complete a directory and the insert position is at the end
					// than we can complete to directory plus slash, so the slash don't have to be typed
					if(seqmentEnd === "" && pathEnd === "" && stat.isDirectory() && fullFilename == file) {
						results.push({
