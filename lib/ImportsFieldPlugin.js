			...
			.tapAsync("ImportsFieldPlugin", (request, resolveContext, callback) => {
				...
				try {
					...
					const error = new Error(
						`${/** @type {Error} */ (err).message} (see imports field in ${
				...
				if (paths.length === 0) {
					// See the matching comment in ExportsFieldPlugin: only this
					// already-failing branch pays for the diagnosis.
					...
					return callback(
						new Error(
							`Package import ${remainingRequest} is not imported from package ${request.descriptionFileRoot} (see imports field in ${request.descriptionFilePath})${
								explanation ? `. In that imports field, ${explanation}` : ""
							}`,
						),
					);
				}
