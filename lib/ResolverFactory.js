...
exports.createResolver = function(options) {
	...
	// Limits module paths to be contained within the given path. Must be an absolute path.
	let jail = options.jail || "";

	if (jail && !jail.endsWith(sep)) {
		jail += sep;
	}
	...
	modules.forEach(item => {
		if (Array.isArray(item))
			plugins.push(
				new ModulesInHierachicDirectoriesPlugin("module", item, "resolve", jail)
			);
		else if (!jail || item.startsWith(jail) || item + sep === jail)
			plugins.push(new ModulesInRootPlugin("module", item, "resolve"));
		else
			throw new Error(
				`The provided module path '${item}' is not within the provided jail path '${jail}'. Please remove it.`
			);
	});
