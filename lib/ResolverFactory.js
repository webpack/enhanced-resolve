 ...
 * @property {boolean=} applyExtensionAliasToExportsField Also apply `extensionAlias` to paths resolved through the package.json `exports` field. Off by default (Node.js-aligned); when enabled, matches TypeScript's behavior for packages that ship TS sources alongside compiled JS.
 ...
 * @property {boolean} applyExtensionAliasToExportsField apply extension alias to exports field targets
...
function createOptions(options) {
	...
	return {
		...
		extensionAlias: options.extensionAlias
			...
			: [],
		applyExtensionAliasToExportsField:
			options.applyExtensionAliasToExportsField || false,
...
module.exports.createResolver = function createResolver(options) {
	...
	if (applyExtensionAliasToExportsField) {
	...
	// resolve-in-package
	const exportsFieldTarget = applyExtensionAliasToExportsField
	...
	// exports-field-relative (opt-in via `applyExtensionAliasToExportsField`):
