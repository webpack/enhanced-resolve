...
const aliasOptionStrategy = require("./AliasOptionStrategy");
...
const fallbackOptionStrategy = require("./FallbackOptionStrategy");
...
exports.createResolver = function(options) {
	...
	aliasFields.forEach(item => {
		plugins.push(
			new AliasFieldPlugin("described-resolve", item, "internal-resolve")
		);
	});
	plugins.push(
	...
	if (resolveToContext) {
		...
		if (alias.length > 0)
			plugins.push(
				new AliasPlugin("file", alias, aliasOptionStrategy, "internal-resolve")
			);
		aliasFields.forEach(item => {
