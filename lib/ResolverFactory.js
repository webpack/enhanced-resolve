...
const CaseSensitivePlugin = require("./CaseSensitivePlugin");
...
module.exports.createResolver = function createResolver(options) {
	...
	if (resolveToContext) {
		...
		// final-file
		// Only register on platforms whose default filesystem is
		// case-insensitive (macOS APFS, Windows NTFS). On case-sensitive
		// filesystems `fs.stat` already implies the basename matches
		// on disk, so the readdir round-trip would be pure overhead.
		// See watchpack#228.
		if (process.platform === "darwin" || process.platform === "win32") {
			plugins.push(new CaseSensitivePlugin("final-file", "final-file"));
		}
