...
exports.createResolver = function(options) {
	...
	for (i = 0; i < alias.length; i++) {
		plugins.push(new AliasPlugin("described-resolve", alias[i], "resolve"));
	}
	plugins.push(new ConcordModulesPlugin("described-resolve", {}, "resolve"));
	for (i = 0; i < aliasFields.length; i++) {
		plugins.push(new AliasFieldPlugin("described-resolve", aliasFields[i], "resolve"));
	}
	...
	// raw-module
	for (i = 0; i < moduleExtensions.length; i++) {
		plugins.push(new ModuleAppendPlugin("raw-module", moduleExtensions[i], "module"));
	}
	...
	for (i = 0; i < modules.length; i++) {
		if(Array.isArray(modules[i]))
			plugins.push(new ModulesInHierachicDirectoriesPlugin("module", modules[i], "resolve"));
		else
			plugins.push(new ModulesInRootPlugin("module", modules[i], "resolve"));
	}
	...
	if(resolveToContext) {
		...
		for (i = 0; i < mainFields.length; i++) {
			plugins.push(new MainFieldPlugin("existing-directory", mainFields[i], "resolve"));
		}
		for (i = 0; i < mainFiles.length; i++) {
			plugins.push(new UseFilePlugin("existing-directory", mainFiles[i], "undescribed-raw-file"));
		}
		...
		for (i = 0; i < extensions.length; i++) {
			plugins.push(new AppendPlugin("raw-file", extensions[i], "file"));
		}

		// file
		for (i = 0; i < alias.length; i++) {
			plugins.push(new AliasPlugin("file", alias[i], "resolve"));
		}
		plugins.push(new ConcordModulesPlugin("file", {}, "resolve"));
		for (i = 0; i < aliasFields.length; i++) {
			plugins.push(new AliasFieldPlugin("file", aliasFields[i], "resolve"));
		}
		if(symlinks)
