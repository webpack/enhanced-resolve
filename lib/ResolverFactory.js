...
const ResolveCache = require("./Cache");
...
/**
 * @param {ResolveCache | Cache | boolean | undefined} unsafeCache unsafeCache option
 * @returns {ResolveCache | Cache | false} resolved cache
 */
function resolveUnsafeCache(unsafeCache) {
	if (!unsafeCache) return false;
	if (unsafeCache instanceof ResolveCache) return unsafeCache;
 ...
 * @property {(ResolveCache | Cache | boolean)=} unsafeCache Use this cache object to unsafely cache the successful requests, or a Cache instance created by createCache()
 ...
 * @property {ResolveCache | Cache | false} unsafeCache unsafe cache
...
function createOptions(options) {
	...
	return {
		...
		fileSystem: options.useSyncFileSystemCalls
			...
			: options.fileSystem,
		unsafeCache: resolveUnsafeCache(options.unsafeCache),
...
module.exports.createResolver = function createResolver(options) {
	...
	// described-resolve
	if (unsafeCache) {
		const unsafeCacheData =
			unsafeCache instanceof ResolveCache ? unsafeCache.data : unsafeCache;
		plugins.push(
			new UnsafeCachePlugin(
				"described-resolve",
				cachePredicate,
				/** @type {import("./UnsafeCachePlugin").Cache} */ (unsafeCacheData),
				cacheWithContext,
				"raw-resolve",
			),
		);
