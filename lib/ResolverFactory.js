...
function normalizePackageMap(packageMap) {
	...
	if (typeof packageMap === "string" || packageMap instanceof URL) {
		return { configFile: toPath(packageMap), packages: null };
	}
...
module.exports.createResolver = function createResolver(options) {
	...
	// A package map is authoritative for bare specifiers: it is consulted
	// before the `modules` directories below and, when it applies, no
	// `node_modules` lookup is attempted.
	if (packageMap) {
		plugins.push(
