...
const CaseSensitivePathPlugin = require("./CaseSensitivePathPlugin");
 ...
 * @property {boolean=} enforceCase Enforce that the file path has the correct casing, even on case insensitive file systems
 */
...
function createOptions(options) {
	...
	return {
		...
		enforceCase: options.enforceCase || false
	};
}
...
exports.createResolver = function(options) {
	...
	resolver.ensureHook("trueCaseFile");
	...
	if (resolveToContext) {
		...
		if (enforceCase)
			plugins.push(
				new CaseSensitivePathPlugin("existing-file", "true-case-file")
			);
		else plugins.push(new NextPlugin("existing-file", "true-case-file"));

		plugins.push(new NextPlugin("true-case-file", "resolved"));
	}
