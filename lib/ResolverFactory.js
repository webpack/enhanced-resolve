...
const { cwd, versions } = require("process");
...
const PackageMapPlugin = require("./PackageMapPlugin");
...
const { PathType, getType, join, toPath } = require("./util/path");
...
/** @typedef {import("./PackageMapPlugin").PackageMapOptions} PackageMapOptions */
 ...
 * only reachable there behind `--experimental-package-map`. This option tracks
 * that specification and may change with it, including in a patch release.
 * @typedef {object} UserPackageMapOptions
 * @property {(string | URL)=} configFile A path, or `file:` `URL` instance, pointing at the package map file. Required together with `packages`, whose relative `url` values are resolved against it
 * @property {PackageMapPackages=} packages An already-parsed `packages` object, used instead of reading `configFile` from disk
 */
 ...
 * @property {(string | URL | UserPackageMapOptions)=} packageMap A Node.js package map file path (or `file:` `URL` instance), or an object with the already-parsed `packages`. When set, bare specifiers resolve through the map instead of `node_modules` (`@experimental`)
 ...
 * @property {PackageMapOptions | null} packageMap package map
...
/**
 * @param {string | URL} configFile the `packageMap` configuration file option
 * @returns {string} the absolute path of the configuration file
 */
function toPackageMapConfigFile(configFile) {
	...
	// The configuration file is the base every package `url` resolves against,
	// so it has to be absolute before it is turned into a URL. Relative paths
	// are taken from the working directory, like `tsconfig` — where there is no
	// working directory to take them from (the resolver also runs in browsers),
	// say so instead of silently resolving against the filesystem root.
...
/**
 * @param {string | URL | UserPackageMapOptions | undefined} packageMap package map option
 * @returns {PackageMapOptions | null} normalized package map, or `null` when the option is unset
 */
function normalizePackageMap(packageMap) {
	if (packageMap === undefined) return null;
	...
	// Relative `url` values are resolved against the configuration file, so an
	// inline `packages` object still needs to say where it would have lived.
	// There is no working directory to fall back on: the resolver runs in
	// browsers too.
	if (packages !== undefined && configFile === undefined) {
		throw new Error(
	...
	return {
		...
		packages: packages === undefined ? null : packages,
	};
}
...
module.exports.createResolver = function createResolver(options) {
	...
	// raw-module
	// A package map is authoritative for every bare specifier that is not a
	// builtin, so it runs ahead of both self-reference and the `modules`
	// directories below: a package that does not declare a dependency on
	// itself must not reach its own `exports` through the self-reference
	// shortcut, and nothing falls back to `node_modules`.
	if (packageMap) {
		plugins.push(
