 ...
 * @property {({[k: string]: any} | null)=} pnpApi
...
exports.createResolver = function(options) {
	...
	const extensions = options.extensions
		? [].concat(/** @type {Array<never>} */ (options.extensions))
		: [".js", ".json", ".node"];
	...
	const modules = mergeFilteredToArray(
		options.modules
			? [].concat(/** @type {Array<never>} */ (options.modules))
			: ["node_modules"],
		item => {
...
function aliasOptionsToArray(alias) {
	return Object.keys(alias).map(key => {
		...
		let obj = { name: key, onlyModule: false };
		/** @type {AliasOptionNewRequest} */
		let request = alias[key];
		if (/\$$/.test(key)) {
			obj.onlyModule = true;
