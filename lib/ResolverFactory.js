...
const { createCachedDirname, createCachedJoin } = require("./util/path");
...
/** @type {WeakMap<Record<string, unknown>, PathCacheFunctions>} */
const pathCacheByOwner = new WeakMap();

/** @type {WeakMap<Record<string, unknown>, Cache>} */
const unsafeCacheByOwner = new WeakMap();

/**
 * @param {Cache | boolean | undefined} unsafeCache unsafeCache option
 * @param {Record<string, unknown>=} owner optional owner object for WeakMap-based lifecycle
 * @returns {Cache | false} resolved cache
 */
function resolveUnsafeCache(unsafeCache, owner) {
	if (!unsafeCache) return false;
	if (typeof unsafeCache === "object") return unsafeCache;
	// unsafeCache === true
	if (owner) {
		let cache = unsafeCacheByOwner.get(owner);
		if (!cache) {
			cache = /** @type {Cache} */ ({});
			unsafeCacheByOwner.set(owner, cache);
		}
		return cache;
	}
	return /** @type {Cache} */ ({});
 ...
 * @property {CacheOptions=} cache Cache configuration options
 */

/**
 * @typedef {object} CacheOptions
 * @property {number=} maxSize Maximum number of entries in join/dirname caches before clearing (0 = unlimited)
 * @property {Record<string, unknown>=} owner An object (e.g. webpack compiler) whose lifetime controls the unsafeCache. When the owner is garbage collected, the associated unsafeCache is also released.
 */
...
function createOptions(options) {
	...
	return {
		...
		fileSystem: options.useSyncFileSystemCalls
			...
			: options.fileSystem,
		unsafeCache: resolveUnsafeCache(options.unsafeCache, cacheOptions.owner),
...
module.exports.createResolver = function createResolver(options) {
	...
	// Apply owner-scoped path cache so multiple resolvers
	// under the same owner share join/dirname caches
	const { owner } = normalizedOptions.cache;
	if (owner) {
		let pathCache = pathCacheByOwner.get(owner);
		if (!pathCache) {
			const maxSize = normalizedOptions.cache.maxSize || 0;
			pathCache = {
				join: createCachedJoin(maxSize),
				dirname: createCachedDirname(maxSize),
			};
			pathCacheByOwner.set(owner, pathCache);
		}
		resolver.join = pathCache.join;
		resolver.dirname = pathCache.dirname;
	}
