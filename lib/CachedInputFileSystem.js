...
class CacheBackend {
	...
	purge(what, options) {
		...
		const exact = options !== undefined && options.exact === true;
		if (exact) {
			if (
				typeof what === "string" ||
	...
	/**
	 * @param {(string | Buffer | URL | number | (string | URL | Buffer | number)[] | Set<string | URL | Buffer | number>)=} what what to purge
	 * @param {{ exact?: boolean }=} options options forwarded to {@link purge}
	 */
	purgeParent(what, options) {
		if (!what) {
			this.purge();
		} else if (
			typeof what === "string" ||
			...
			const strWhat = typeof what !== "string" ? what.toString() : what;
			this.purge(dirname(strWhat), options);
		} else {
			const set = new Set();
			for (const item of what) {
				const strItem = typeof item !== "string" ? item.toString() : item;
...
module.exports = class CachedInputFileSystem {
	...
	purge(what, options) {
		...
		this._readdirBackend.purgeParent(what, options);
