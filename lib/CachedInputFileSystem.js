...
// Shared options object passed to fs.stat / fs.statSync (and lstat) on the
// no-caller-options path. Frozen so passing it cannot be mutated by a
// misbehaving fs implementation.
const STAT_OPTS = Object.freeze({ throwIfNoEntry: false });

/**
 * Build an ENOENT-shaped error without paying for a captured stack trace.
 *
 * V8's `new Error(...)` invokes `Error.captureStackTrace`, which is the
 * dominant per-call cost when fs.statSync would otherwise throw on a
 * missing path. Cache consumers only ever read `.code` / `.path` /
 * `.message` on these errors (`instanceof Error` still works because we
 * inherit from `Error.prototype`), so the stack is never observed.
 * @param {string} path path that produced the ENOENT
 * @param {string} syscall syscall name (e.g. "stat", "lstat")
 * @returns {NodeJS.ErrnoException} cheap ENOENT error
 */
function createENOENT(path, syscall) {
	const err =
		/** @type {NodeJS.ErrnoException} */
		(Object.create(Error.prototype));
	err.name = "Error";
	err.message = `ENOENT: no such file or directory, ${syscall} '${path}'`;
	err.code = "ENOENT";
	err.errno = -2;
	err.syscall = syscall;
	err.path = path;
...
/**
 * Build a stat/lstat provider pair that uses Node's `throwIfNoEntry: false`
 * option (added in Node 14.17) on the no-options hot path, synthesizing a
 * cheap ENOENT error on miss instead of letting V8 capture a stack trace.
 *
 * Feature-detected at construction: when the underlying fs ignores the
 * option (older Node, exotic fs implementations) the probe still throws
 * normally, and we fall back to the unwrapped providers — no observable
 * change. When the option is honored, every first-miss `stat`/`lstat`
 * skips the stack-trace allocation.
 * @param {EXPECTED_FUNCTION | undefined} asyncProvider async stat/lstat method on the fs
 * @param {EXPECTED_FUNCTION | undefined} syncProvider sync stat/lstat method on the fs
 * @param {BaseFileSystem} fs fs the providers belong to
 * @param {string} syscall syscall name for the synthesized error
 * @returns {{ asyncProvider: EXPECTED_FUNCTION | undefined, syncProvider: EXPECTED_FUNCTION | undefined }} wrapped pair
 */
const wrapStatProviders = (asyncProvider, syncProvider, fs, syscall) => {
	// Probe needs a real provider plus a path that's guaranteed to be
	// missing. A NUL byte in the middle of the path is rejected by every
	// real filesystem with ENOENT, so it's a safe "negative" probe.
	let optionSupported = false;
	if (syncProvider) {
		// Path must be virtually guaranteed not to exist AND free of NUL
		// bytes (Node rejects NUL with TypeError before consulting the
		// option, which would mask whether it's honored).
		const probePath = `/enhanced-resolve-probe-${Date.now()}-${Math.random().toString(36).slice(2)}`;
		try {
			const r = syncProvider.call(fs, probePath, STAT_OPTS);
			optionSupported = r === undefined;
		} catch (_err) {
	...
	if (!optionSupported) {
		return { asyncProvider, syncProvider };
	}

	/**
	 * @param {string} path path
	 * @param {object=} options caller-supplied options (pass-through)
	 * @returns {EXPECTED_ANY} stat result
	 */
	const wrappedSync = (path, options) => {
		if (options) {
			return /** @type {EXPECTED_FUNCTION} */ (syncProvider).call(
				fs,
				path,
				options,
			);
		}
		const r = /** @type {EXPECTED_FUNCTION} */ (syncProvider).call(
			fs,
			path,
			STAT_OPTS,
		);
		if (r === undefined) throw createENOENT(path, syscall);
		return r;
	};

	/**
	 * @param {string} path path
	 * @param {object | EXPECTED_FUNCTION=} options caller options or callback
	 * @param {EXPECTED_FUNCTION=} callback callback
	 * @returns {void}
	 */
	const wrappedAsync = (path, options, callback) => {
		if (typeof options === "function") {
			callback = /** @type {EXPECTED_FUNCTION} */ (options);
			options = undefined;
		}
		if (options) {
			/** @type {EXPECTED_FUNCTION} */
			(asyncProvider).call(fs, path, options, callback);
			return;
		}
		/** @type {EXPECTED_FUNCTION} */
		(asyncProvider).call(
	...
	return {
		asyncProvider: asyncProvider ? wrappedAsync : undefined,
		syncProvider: syncProvider ? wrappedSync : undefined,
	};
};
...
module.exports = class CachedInputFileSystem {
	...
	constructor(fileSystem, duration) {
		...
		const lstatProviders = wrapStatProviders(
		...
		this._lstatBackend = createBackend(
			duration,
			lstatProviders.asyncProvider,
			lstatProviders.syncProvider,
			this.fileSystem,
		);
		...
		const statProviders = wrapStatProviders(
		...
		this._statBackend = createBackend(
			duration,
			statProviders.asyncProvider,
			statProviders.syncProvider,
			this.fileSystem,
		);
