...
/**
 * @template T
 * @param {((err?: Error, result?: any) => void)[]} callbacks callbacks
...
class CacheBackend {
	...
	constructor(duration, provider, syncProvider, providerContext) {
		...
		this._syncProvider = syncProvider;
		this._providerContext = providerContext;
		/** @type {Map<string, ((err?: Error, result?: any) => void)[]>} */
		this._activeAsyncOperations = new Map();
	 ...
	 * @param {(err?: Error, arg?: any) => void} callback callback
	 * @returns {void}
	 */
	provide(path, options, callback) {
		...
		this._provider.call(this._providerContext, path, (err, result) => {
			...
			runCallbacks(
				/** @type {((err?: Error, result?: any) => void)[]} */ (callbacks),
				err,
				result
			);
		});
 ...
 * @param {any} syncProvider sync provider
 * @param {any} providerContext provider context
