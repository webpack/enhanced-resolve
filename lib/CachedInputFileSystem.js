...
/**
 * @template T
 * @param {Callback<T>[]} callbacks callbacks
...
class CacheBackend {
	 ...
	 * @param {string} path path
	 * @param {any} options options
	 * @param {Callback<any>} callback callback
	 * @returns {void}
	 */
	provide(path, options, callback) {
		...
		this._provider.call(
			...
			(err, result) => {
				...
				runCallbacks(/** @type {Callback<any>[]} */ (callbacks), err, result);
			}
		);
	}
