	...
	resolver.plugin("module", function(request, callback) {
		...
		(function next() {
			for(;i < keys.length; i++) {
				...
				var rightSide = /\$$/.test(aliasName);
				if(rightSide) aliasName = aliasName.substr(0, aliasName.length-1);
				if((!rightSide && request.request.indexOf(aliasName + "/") === 0) ||
				   (rightSide && request.request.substr(-aliasName.length) === aliasName) ||
				   (request.request === aliasName)) {
