		...
		resolver
			...
			.tapAsync("ExportsFieldPlugin", (request, resolveContext, callback) => {
				...
				try {
					...
					// Name the package.json in the error itself, not only in the
					// log: a malformed field is a mistake in someone's package,
					// and without the path there is nothing to go on. The
					// original message is kept verbatim inside this one.
					const error = new Error(
						`${/** @type {Error} */ (err).message} (see exports field in ${
				...
				if (paths.length === 0) {
					...
					// Nothing matched. The usual reason is that the package
					// really does not export this path, but a field whose
					// conditions wrap subpaths matches nothing either - and
					// then the message above points at the request rather than
					// at the mistake. Only this failing branch pays for the
					// check. See https://github.com/webpack/enhanced-resolve/issues/325.
					...
					return callback(
						new Error(
							`"${remainingRequest}" is not exported under ${conditionsStr} from package ${request.descriptionFileRoot} (see exports field in ${request.descriptionFilePath})${
								explanation ? `. In that exports field, ${explanation}` : ""
							}`,
						),
					);
				}
