			...
			.tapAsync("ExportsFieldPlugin", (request, resolveContext, callback) => {
				...
				forEachBail(
					...
					(path, callback, i) => {
						...
						// `relativePath` always starts with "./" here (checked above),
						// so slicing the dot-slash prefix is safe and we can do it
						// once instead of per regex test.
						const segmentCheckTarget = relativePath.slice(2);
						if (
							invalidSegmentRegEx.exec(segmentCheckTarget) !== null &&
							deprecatedInvalidSegmentRegEx.test(segmentCheckTarget)
						) {
							if (paths.length === i) {
								return callback(
									new Error(
