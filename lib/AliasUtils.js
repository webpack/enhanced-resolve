	...
	forEachBail(
		...
		(item, callback) => {
			...
			if (matchRequest || matchWildcard) {
				// Hoist per-item work out of the per-alias closure. When
				// `item.alias` is an array, `resolveWithAlias` runs once per
				// element; before, each run recomputed the same prefix/suffix
				// checks and the same `innerRequest.slice(...)`. These values
				// depend only on `innerRequest` and `item`, both stable here.
				...
				const resolveWithAlias = (alias, callback) => {
					...
					if (wildcardMatch !== undefined) {
						newRequestStr = alias.toString().replace("*", wildcardMatch);
					}
					...
					if (
						...
						newRequestStr = alias + /** @type {string} */ (remainingRequest);
					}
