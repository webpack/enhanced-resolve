...
function compileAliasOptions(resolver, options) {
	...
	return { all, byFirstChar, hasAnyFirstChar };
}
...
function aliasResolveHandler(
	...
	// Dispatch through the first-char-code bucket when possible: aliases
	// whose `name` starts with a different char cannot match, so skipping
	// them up front saves the per-option `forEachBail` overhead entirely.
	// When any option has an empty-prefix wildcard (`firstCharCode === -1`,
	// e.g. `name: "*"`), it can match any first char — fall back to the
	// full declaration-ordered list to preserve the existing first-match
	// semantics across the otherwise-disjoint buckets.
	let scan;
	if (options.hasAnyFirstChar) {
		scan = options.all;
	} else {
