 ...
 *
 * Direct string concatenation — ~3–5× faster than the previous
 * `JSON.stringify({ ... })` form because we skip the temporary object
 * and the JSON serializer for fields that are already plain strings.
 * The cache lives in-memory for the lifetime of the resolver and its
 * key shape is opaque to consumers, so the format change is invisible.
