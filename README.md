...
| cache               | {}                          | Cache lifecycle options. `cache.maxSize` (number, default `0` meaning unlimited) sets the maximum number of entries in path caches (join/dirname) before clearing. `cache.owner` (object) ties cache lifetime to an external object (e.g. webpack compiler) via WeakMap — when the owner is garbage collected, all associated caches (path caches and unsafeCache) are released automatically. |
