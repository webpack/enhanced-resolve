...
| unsafeCache         | false                       | Enable unsafe caching of successful requests. Can be `true` (create a plain cache object), a plain object (user-managed), or a `Cache` instance created by `createCache()` for managed lifecycle and size limits.                                                                           |

#### Cache

Use `createCache()` to create a managed cache instance that controls the lifecycle of both the unsafe resolve cache and internal path caches (join/dirname).
...
// Basic usage — maxSize limits internal path cache entries (0 = unlimited)
const basicCache = createCache({ maxSize: 5000 });

// With owner — same owner always returns the same Cache instance via WeakMap.
// When the owner is garbage collected, the cache is released automatically.
const cache = createCache({ owner: compiler, maxSize: 5000 });
...
| ------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| maxSize | 0       | Maximum number of entries in path caches (join/dirname) before clearing. `0` means unlimited.                                           |
| owner   | -       | An object (e.g. webpack compiler) whose lifetime controls the cache. Same owner returns the same Cache instance; GC releases the cache. |
